...
sample_B0 <- function(B0, E, h, b0_B0, iV_B0) {
    ...
    for (i in 1:n) {
        iOmega_i <- 1/exp(h[, i])
        Kbi <- iV_B0 + crossprod(E, iOmega_i*E)
        ...
        # nv = n forces the full right-singular basis: the last column spans
        # the null space of the (n-1) x n matrix (MATLAB's svd(.,0) here)
        V_svd <- svd(B0_no_i, nu = 0, nv = n)$v
        w <- V_svd[, n]
        ...
        mu_an <- xi_hat[1]
        disc <- sqrt(mu_an^2 + 4)
        # plogis(-x) = 1/(1 + exp(x)) avoids overflow for large mu_an*T
        if (runif(1) < plogis(-2*mu_an*T)) {   # weight on the negative mode
            m_an <- (mu_an - disc)/2
        } else {
            m_an <- (mu_an + disc)/2
        }
        ...
        bi_new <- as.numeric(backsolve(t(Ci), Vmat %*% xi))   # t(Ci) is upper
