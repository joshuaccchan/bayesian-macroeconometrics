...
Minn_indep <- function(p, kappa1, kappa2, kappa3, Y0, Yt, rw) {
    ...
    U_hat <- matrix(0, T, n)
    p0 <- nrow(Y0)
    tmpY <- rbind(Y0[(p0-p+1):p0, , drop = FALSE], Yt)
    for (i in 1:n) {
        Z_ar <- matrix(1, T, p+1)
        for (l in 1:p) {
            Z_ar[, l+1] <- tmpY[(p-l+1):(p+T-l), i]
        }
        b_ar <- qr.solve(Z_ar, Yt[, i])       # MATLAB's Z_ar \ Yt(:,i) (OLS)
    ...
    list(beta_Minn = beta_Minn, V_Minn = V_Minn, s2_hat = s2_hat, U_hat = U_hat)
}
