...
# beta_Minn: (n*k)-vector prior mean of beta = vec(A), k = 1 + n*p
# V_Minn:    (n*k)-vector of diagonal entries of the prior covariance of beta
# s2_hat:    n-vector of univariate AR(p) residual variances (s_i^2)
# U_hat:     T x n matrix of univariate AR(p) residuals

Minn_indep <- function(p, kappa1, kappa2, kappa3, Y0, Yt, rw) {
    ...
    for (i in 1:n) {
        Z_ar <- matrix(1, T, p+1)
        for (l in 1:p) {
            Z_ar[, l+1] <- tmpY[(p-l+1):(nrow(tmpY)-l), i]
        }
        b_ar <- qr.solve(Z_ar, Yt[, i])
        U_hat[, i] <- Yt[, i] - as.numeric(Z_ar %*% b_ar)
        s2[i] <- mean(U_hat[, i]^2)
    }
    s2_hat <- s2
    ...
    list(beta_Minn = beta_Minn, V_Minn = V_Minn, s2_hat = s2_hat,
         U_hat = U_hat)
}
