...
# phi:    n-vector of current persistence
# sigh2:  n-vector of current innovation variance
# phi_0:  scalar; prior mean of phi_i
# V_phi:  scalar; prior variance of phi_i
# nu_h:   n-vector IG shape parameter
# S_h:    n-vector IG scale parameter
#
# Outputs (returned in a list):
# phi:    n-vector updated persistence
# sigh2:  n-vector updated innovation variance
...
sample_SVAR1para <- function(h, phi, sigh2, phi_0, V_phi, nu_h, S_h) {
    ...
    for (i in 1:n) {
        ...
        sigh2[i] <- 1/rgamma(1, shape = nu_h[i] + T/2,
                             scale = 1/(S_h[i] + ss/2))
    }
