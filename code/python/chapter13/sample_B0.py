...
def sample_B0(B0, E, h, b0_B0, iV_B0):
    ...
    for i in range(n):
        # Kbi = iV_B0 + E' Omega_{h_i}^{-1} E with Omega_{h_i} = diag(exp(h(:,i)))
        Kbi = iV_B0 + (E/np.exp(h[:, i])[:, None]).T @ E
        ...
        _, _, Vt_svd = np.linalg.svd(B0_no_i)
        w = Vt_svd[-1, :]
        ...
        # cap the exponent to avoid overflow warnings; the weight is 0 either way
        if np.random.rand() < 1/(1 + np.exp(min(2*mu_an*T, 709.0))):
            m_an = (mu_an - disc)/2             # weight on the negative mode
        else:
            m_an = (mu_an + disc)/2
        xi[0] = m_an + np.sqrt(m_an**2/(T*(1 + m_an**2)))*np.random.randn()
        for j in range(1, n):
            xi[j] = xi_hat[j] + (1/np.sqrt(T))*np.random.randn()
