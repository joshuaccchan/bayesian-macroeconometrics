...
for isim in range(nsim + burnin):
    # sample beta
    X_tilde = solve_banded((1, 0), Hpsi_ab, X)
    y_tilde = solve_banded((1, 0), Hpsi_ab, y)
    Dbeta = np.linalg.solve(iVbeta + X_tilde.T @ X_tilde/sig2, np.eye(k))
    beta_hat = Dbeta @ (iVbeta @ beta0 + X_tilde.T @ y_tilde/sig2)
    beta = beta_hat + np.linalg.cholesky(Dbeta) @ np.random.randn(k)

    # sample sig2
    e = y - X @ beta
    u = solve_banded((1, 0), Hpsi_ab, e)
...
print("Posterior mean of [beta', sig2, psi]:")
print(theta_mean)
