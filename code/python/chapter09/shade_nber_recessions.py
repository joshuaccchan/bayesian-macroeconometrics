...
#                (axvspan spans the full height of the axes; the drivers pass
#                the current y-limits, so the result matches the MATLAB patch)
...
def shade_nber_recessions(ymin, ymax):
    ...
    for i in range(R.shape[0]):
        x1 = dec(R[i, 0], R[i, 1])
        x2 = dec(R[i, 2], R[i, 3])
        # zorder=0 keeps the shading behind lines drawn afterwards
        ax.axvspan(x1, x2, color=(0.9, 0.9, 0.9), zorder=0)
